...
function MQTT.client:destroy()                                    -- Public API
  MQTT.Utility.debug("MQTT.client:destroy()")
	...
	print("MQTT.client:destroy() from "..where)
...
function MQTT.client:disconnect()                                 -- Public API
  MQTT.Utility.debug("MQTT.client:disconnect()")
	...
	local where = info.source..':'..info.currentline
	if where:sub(1,1) == '@' then where = where:sub(2) end
	print("MQTT.client:disconnect() from "..where)

  if (self.connected) then
    self.connected = false	-- Avoid doing this recursively
    self:message_write(MQTT.message.TYPE_DISCONNECT, nil)
    self.socket_client:close()
    self.connected = false	-- And be a bit redundant!
  else
...
function MQTT.client:message_write(                             -- Internal API
  ...
  payload,       -- string
  addedControl)	 -- bits to add to the header control byte (defaults to 0)
                 -- return: nil or error message

-- TODO: Complete implementation of fixed header byte 1

  local message = string.char(MQTT.Utility.shift_left(message_type, 4) + (addedControl or 0))
...
-- TODO: Enable Subscribe with QOS and deal with PUBACK, etc.
    ...
    local retain = MQTT.Utility.shift_left(MQTT.Utility.shift_right(message_type_flags, 1), 1) ~= message_type_flags
...
function MQTT.client:publish(                                     -- Public API
  ...
  self:message_write(MQTT.message.TYPE_PUBLISH, message, retain and 1 or 0)
end
...
function MQTT.client:subscribe(                                   -- Public API
  ...
  if not self:message_write(MQTT.message.TYPE_SUBSCRIBE, message) then
...
function MQTT.client:unsubscribe(                                 -- Public API
  ...
  if not self:message_write(MQTT.message.TYPE_UNSUBSCRIBE, message) then
