...
local function closeScene()
	...
	if name == "buttons_scene" then
		...
		--print("Closing button_scene from "..tostring(where))
...
--	buttonView = layer
	...
	for i,g in pairs(mapScene.getGPXs()) do
		...
		table.insert(entries, {label=g.name, value=i, labelcolor={0,0,0}, --g.color,
								detail=string.format("%d segments %d points @ %d - %s",#g, totalPoints,g.lineWidth or 1,colors:getColorName(g.color)),
								detailcolor=g.color})
	end
...
print("gpxButton:"..tostring(mapScene).." "..type(mapScene.showGPXs).." "..tostring(mapScene.hideGPXs))
	...
	mapsButton = Button {
			...
			parent = buttonView, priority=2000000000,
			onClick = function()
							SceneManager:openScene("maps_scene", {animation = "popIn", backAnimation = "popOut", })
							resetTimer()
						end,
	}
	mapsButton:setScl(config.Screen.scale,config.Screen.scale,1)
	
local ssid = 1
local objs = {}
local abortToast
local function tileStatus(x,y,z,status)
	if status ~= "OK" and status ~= "move" then print(string.format("Spiral %s %d/%d/%d", status and status or "FAILED", z, x, y)) end
	if not status then
		toast.new(string.format("%d/%d/%d Failed", x, y, z),1000)
	else
		if status:sub(1,5) == "start" then
			--toast.new(status)
			return
		elseif status:sub(1,4) == "done" or status:sub(1,7) == "aborted" then
			if abortToast then toast.destroy(abortToast, true) end
			toast.new("Tap to Clear Objects\n"..status, nil, function()
	local tm = os.date('!*t')
	local header = config.StationID..">"..config.ToCall..":"
	for k,t in pairs(objs) do
		local packet = header..APRS:ObjectHHMMSS(k, tm.hour, tm.min, tm.sec,
											{lat=t.lat, lon=t.lon, symbol='/q', comment=''}, true)	-- a kill packet
		stations.packetReceived(packet)	-- remove it from our local map
	end
													end)
			return
		end
	end
	local lat, lon = tilemgr:osmTileLatLon(math.floor(x)+0.5, math.floor(y)+0.5, z)
	local dlat, dlon = tilemgr:osmTileSizeDegrees(lat, lon, z)

	local s = {}
	table.insert(s,{lat=lat-dlat/2,lon=lon-dlon/2})
	table.insert(s,{lat=lat+dlat/2,lon=lon-dlon/2})
	table.insert(s,{lat=lat+dlat/2,lon=lon+dlon/2})
	table.insert(s,{lat=lat-dlat/2,lon=lon+dlon/2})
	table.insert(s,{lat=lat-dlat/2,lon=lon-dlon/2})
	local scale = math.max(dlat,dlon)/44	-- Maximum delta over the 44 options
	...
	local comment = tostring(status).." }j0"..temp.."{!W00!"	-- j for blue 1 for just an outline
	local tm = os.date('!*t')
	local header = config.StationID..">"..config.ToCall..":"
	local packet = header..APRS:ObjectHHMMSS("FETCH-ER", tm.hour, tm.min, tm.sec,
										{lat=lat, lon=lon, symbol='\\O', comment=comment})	-- \O=shuttle /q or \A
	objs["FETCH-ER"] = { lat=lat, lon=lon }
	--print(packet)
	stations.packetReceived(packet)	-- put it on our local map
	if status ~= "move" and status ~= "OK" then
		local ID = string.format("FETCH-%02d",ssid)
		local packet2 = header..APRS:ObjectHHMMSS(ID, tm.hour, tm.min, tm.sec,
											{lat=lat, lon=lon, symbol='/q', comment=comment})	-- /q or \A
		objs[ID] = { lat=lat, lon=lon }
		stations.packetReceived(packet2)	-- put it on our local map
		ssid = ssid + 1
		if ssid > 99 then ssid = 1 end
	end
end

	prefetchButton = Button {
			text = "Fetch+4z",
			red=buttonAlpha, green=buttonAlpha, blue=buttonAlpha, alpha=buttonAlpha,
			size = {120, 66},
			parent = buttonView, priority=2000000000,
...
local range = milesToKm((horzDistance+vertDistance)/2)
local clat, clon = osmTiles:getCenter()
local abort = false
local zmax = math.min(osmTiles:getZoom()+4,17)
local count, vmin, vmax = tilemgr:spiralPreloadTiles(clat,clon,range,zmax,function(x,y,z,status) tileStatus(x,y,z,status) return abort end)
if count then
	abortToast = toast.new(string.format("Loading %d Tiles\nZoom %d -> %d -> 0\n\nTap HERE to abort", count, zmax, osmTiles:getZoom()), nil, function() abortToast = toast.new("Aborting...") abort = true end)
else toast.new(vmin, 5000)
end
							resetTimer()
						end,
	}
	prefetchButton:setScl(config.Screen.scale,config.Screen.scale,1)
	...
	if config.StationID == 'KJ4ERJ-DV' then
		resourcesButton = Button {
			...
			parent = buttonView, priority=2000000000,
			onClick = function()
							SceneManager:openNextScene("resources_scene", {animation = "popIn", backAnimation = "popOut", })
						end,
		}
		resourcesButton:setScl(config.Screen.scale,config.Screen.scale,1)
	end
...
--        textSize = 24,
	...
	if whoStation == myStation then
		local thatStation = stations:getStation("MY-IP")
		if thatStation then whoStation = thatStation end
	end
	local whoButton
	if whoStation ~= myStation then
		local whoLabel
...
print("MoveME:result="..tostring(result))
					if result == MOAIDialogAndroid.DIALOG_RESULT_POSITIVE then
						...
						moveME(tileGroup.lat, tileGroup.lon)
						stations:updateCenterStation()	-- Actually move the symbol
    ...
    if walkButton then
		...
		if pulseButton then
			pulseButton:setLeft(10) pulseButton:setTop(walkButton:getBottom()+10)
			mapsButton:setLeft(pulseButton:getRight()) mapsButton:setTop(pulseButton:getTop())
		else
			mapsButton:setLeft(10) mapsButton:setTop(walkButton:getBottom()+10)
		end
		...
		if pulseButton then
			pulseButton:setLeft(10) pulseButton:setTop(configureButton:getBottom()+10)
			mapsButton:setLeft(pulseButton:getRight()) mapsButton:setTop(pulseButton:getTop())
		else
			mapsButton:setLeft(10) mapsButton:setTop(configureButton:getBottom()+10)
		end
		if openButton then openButton:setLeft(10) openButton:setTop(walkButton:getBottom()+10)
			if closeButton then closeButton:setLeft(10) closeButton:setTop(openButton:getBottom()+10)
				if queryButton then queryButton:setLeft(10) queryButton:setTop(closeButton:getBottom()+10) end
	...
	prefetchButton:setLeft(mapsButton:getRight()) prefetchButton:setTop(mapsButton:getTop())
	...
	if resourcesButton then
		resourcesButton:setRight(messageButton:getLeft()); resourcesButton:setTop(messageButton:getBottom()+10)
	end
...
local function resizeHandler ( width, height )
	...
	print('buttons_scene:onResize:'..tostring(width)..'x'..tostring(height))
...
function onKeyDown(event)
    print("Buttons:onKeyDown:"..printableTable("event",event))
	if event.key then
...
function onCreate(e)
	print('Buttons:onCreate, resizeHandler='..tostring(resizeHandler))
...
--[[
	...
	titleGroup:addEventListener("touchUp",
			function(e)
				--print("Tapped Button:TitleGroup:"..printableTable("e",e))
				if e.isTap then
					closeScene()
				else print("Not isTap, not closing buttons_scene")
				end
			end)
end
