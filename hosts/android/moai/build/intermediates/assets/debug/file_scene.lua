...
function backHandler()
	...
	local function human(c)
		...
		if c < divisor then
			return tostring(c)
		elseif c < 10*divisor then
			return string.format("%.2fK", c/divisor)
		elseif c < 100*divisor then
			return string.format("%.1fK", c/divisor)
		elseif c < divisor*divisor then
			return string.format("%.0fK", c/divisor)

		elseif c < 10*divisor*divisor then
			return string.format("%.2fM", c/divisor/divisor)
		elseif c < 100*divisor*divisor then
			return string.format("%.1fM", c/divisor/divisor)
		elseif c < divisor*divisor*divisor then
			return string.format("%.0fM", c/divisor/divisor)
			
		elseif c < 10*divisor*divisor*divisor then
			return string.format("%.2fG", c/divisor/divisor/divisor)
		elseif c < 100*divisor*divisor*divisor then
			return string.format("%.1fG", c/divisor/divisor/divisor)
		elseif c < divisor*divisor*divisor*divisor then
			return string.format("%.0fG", c/divisor/divisor/divisor)
			
		elseif c < 10*divisor*divisor*divisor*divisor then
			return string.format("%.2fT", c/divisor/divisor/divisor/divisor)
		elseif c < 100*divisor*divisor*divisor*divisor then
			return string.format("%.1fT", c/divisor/divisor/divisor/divisor)
		elseif c < divisor*divisor*divisor*divisor*divisor then
			return string.format("%.0fT", c/divisor/divisor/divisor/divisor)
			
		else return string.format("%.0fT", c/divisor/divisor/divisor/divisor)
		end
	end

	local function buildChooser(dir, newValue, confirm, withExtra)
		...
		if not (dir == "/" or dir:sub(2,-1) == ":/") then
			table.insert(entries, {label="..", value=dir.."/..", detail=MOAIFileSystem.getAbsoluteDirectoryPath(dir.."/.."),
						tapBack = function(v) buildChooser(v, newValue, confirm, withExtra) end})
		end
		if withExtra and type(withExtra) == 'table' and withExtra.label and withExtra.value then
			table.insert(entries, {label=withExtra.label, value=withExtra.value, detail=withExtra.detail,
						tapBack = function(v)
									newValue(v)
									backHandler()
								end})
		...
		for i, f in pairs(files) do
			if not match or f:match(match) then
				...
				if attrs then
					if attrs.size then detail = detail..human(attrs.size) end
...
function onCreate(params)
	...
	buildChooser(params.dir or ".", newValue, confirm, params.extra)
end
