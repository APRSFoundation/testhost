...
function APRS:unpackMMDDMMSS(value, from)
	local s, e, one, two, three, four = value:find("^(%d%d)(%d%d)(%d%d)(%d%d)$")
	if not s then return nil, 'APRS:unpackMMDDMMSS('..value..') Invalid' end
	local result = {}
	result.which = 'w'
	result.month, result.day, result.hour, result.minute = one, two, three, four
	return result
	--return nil, 'APRS:unpackMMDDMMSS('..value..') Invalid Time Type '
end
...
local function parseWeather(info, comment)	-- Return updated comment
	local valids = { c="windDirection",	-- Degrees
					s="windSpeed",	-- mph
					g="windGust",	-- mph
					t="temperature",	-- degrees Fahrenheit (may be negative)
					r="rainfallHour",	-- hundredths of an inch
					p="rainfall24Hour",	-- hundredths of an inch
					P="rainfallMidnight",	-- hundredths of an inch
					h="humidity",	-- percentage
					l="luminosity",	-- luminosity >= 1000
					L="luminosity",	-- luminisity < 1000
					F="waterHeight",	-- feet
					f="waterHeight",	-- meters
					b="barometer",	-- tenths of millibars/tenths of hPascal
					V="batteryVolts",	-- tenths of volts
					X="radiation" }	-- nanosieverts mms (mm*10^s)
	valids["#"] = "rainfallCounts"
	--print(string.format("%s needs weather from %s", info.src, comment))
	while comment ~= "" do
		local s, e, what, svalue = comment:find("^(.)([%-%d%s%.]+)")
		--print(string.format("Weather.find %s %s %s %s from '%s'", tostring(s), tostring(e), tostring(what), tostring(svalue), comment))
		if not svalue then break end
		if not valids[what] then break end
		local value = tonumber(svalue)
		if value then	-- Valid numbers might need a conversion
			if what == 'l' then
				value = value + 1000
			elseif what == 'f' then
			...
			elseif what == 'V' then
				value = value / 10
			elseif what == 'X' then
				local units, fraction = math.modf(value / 10)
				value = units * math.pow(10, fraction*10)
			end
			info.weather[valids[what]] = value
		end
		comment = comment:sub(e+1)
		-- print(string.format("Got weather term %s(%s) value '%s'(%s) comment now '%s'", what, valids[what], svalue, tostring(value), comment))
	end
	-- print(string.format("%s comment after weather %s, %s", info.src, comment, printableTable("weather", info.weather)))
	return comment
end
	...
	elseif info.packetType == '/' or info.packetType == '@' then
		...
		if info.time then
			...
			if first < '0' or first > '9' then
				...
				if info.symbol then
					comment = comment:sub(22)
-- KJ4OVQ-9>APOT30,WIDE1-1,WIDE2-1,qAO,KJ4ERJ-S5:/145732h/@@/_:*n_v=:GSafari Van
-- KJ4ERJ-12>APWA01,TCPIP*,qAC,T2BC:@141038h2759.81N/08039.55W$Time 0:00:00 (0/1800) APRSISMO de KJ4ERJ
				else print('Invalid Compressed Coordinate from('..info.src..') in '..comment)
				end
			else
	...
	elseif info.packetType == '`' or info.packetType == "'" then
		info.lat, info.lon, info.symbol, info.course, info.speed, info.miceMessage, info.error = APRS:unpackMicECoordinate(info.dst, comment:sub(2,9), info.src)
...
yaesuPlatforms["_0"] = 'Yaesu FT-3D'
yaesuPlatforms["_1"] = 'Yaesu FTM-300D'
				local foundit = false
...
--print('Possible message('..tostring(comment)..')')
		if #comment >= 11 and comment:sub(11,11) == ':' then
			...
			if info.src == info.msg.addressee then	-- Talking to myself?
				local telemetryDefs = { PARM=true, UNIT=true, EQNS=true, BITS=true }
				if #info.msg.text > 5 and info.msg.text:sub(5,5) == '.' and telemetryDefs[info.msg.text:sub(1,4)] then
					info.msg.telemetryDef = true
--print ('Message from ('..info.src..') to ('..info.msg.addressee..') telemetryDef('..info.msg.text..')')
				end
			end
		end
	...
	elseif info.packetType == 'T' then	-- Telemetry
		if comment:sub(2,2) == '#' then	-- Anything else is probably plain text status starting with T
			...
			local found
			--if info.src == "KJ4ERJ-E2" then print(info.src..tostring(comment)) end
			...
			--if info.src == "KJ4ERJ-E2" then print(printableTable(info.src, info.telemetry.values)) end
		else
...
--[[		if (ValueCount < 5)
	...
	elseif info.packetType == '_' then	-- Positionless weather report
		info.symbol = "/_"		-- _ = Weather station
		info.time, info.error = APRS:unpackMMDDMMSS(comment:sub(1,8), info.src)
		if info.time then
			comment = comment:sub(9)
			comment = parseWeather(info, comment)
		end
	else
...
-- Especially before the !DAO! is checked as the telemetry contain a !xxx!
	do
		local s,e,c,b91 = comment:find("(.*)|(....+)|")
		if s and e and c and b91 then
			--print(string.format("%s has BASE91 COMMENT TELEMETRY (%s) %d-%d in %s", info.src, b91, s, e, comment))
			if #b91 > 4 and mybits.band(#b91,1) == 0 then	-- length inside, must be even and > 4
				local seq = base91Decode(b91:sub(1,2))
				if seq then
					local allOK = true
					info.telemetry = { seq=seq }
					info.telemetry.values = {}
					for n = 1, #b91/2-1	do --  ss1122334455dd
						local v = base91Decode(b91:sub(n*2+1,n*2+2))
						if v then
							if n== 6 then
								info.telemetry.digital = v
							else
								info.telemetry.values[#info.telemetry.values+1] = v
							end
						else allOK = false
						end
					end
					if allOK then
						comment = comment:sub(s,s+#c-1)..comment:sub(e+1)
					else
						info.telemetry = nil
						print(string.format("%s has BAD BASE91 COMMENT TELEMETRY (%s) %d-%d in %s", info.src, b91, s, e, comment))
					end
				end
			end
		end
	...
	if info.lat and info.lon then	-- Position-based packets might have data extension
		do
			...
			if D then
				info.PHG = {}
				if comment:sub(s+1,s+7) == 'PHG0000' then	-- "magic" reset string
					...
					comment = comment:sub(1,s-1)..comment:sub(e+1)
				end
			end
		end
		do
		...
		if comment:sub(4,4) == '/' or comment:sub(4,4) == '1' then
			local symbol = (info.symbol or '??')
			if symbol == "\\l" then	-- Area Object Descriptor
				print('Station('..info.src..') has AreaObject('..comment..")")
				local s,e, T, latoff, C, lonoff = comment:match("^(%c)(%d%d)(%c%d)(%d%d)")
				if lonoff then
					print('Station('..info.src..') has CONFIRMED AreaObject('..comment..")")
				end
--[[
;OBJECTVVV*ddmm.hhN\dddmm.hhW l Tyy/Cxx{wid}
where:
...
/C Object Color Intensity
		...
		if comment:sub(4,4) == '/' then
			...
			or symbol:sub(2,2) == 'H'	-- Hazards have this
			or symbol:sub(2,2) == 'w'		-- as does water?
			or symbol:sub(2,2) == '_' then	-- Weather direction/speed
				if not info.weather then info.weather = {} end
				if l ~= '   ' and l ~= '...' and r ~= '   ' and r ~= '...' then
					info.weather.direction = tonumber(l)
					info.weather.windspeed = tonumber(r)
				end
				comment = comment:sub(8)
				comment = parseWeather(info, comment)
			elseif symbol:sub(2,2) == '@' then	-- Storm (Tropical Storm/Tropical Depression/Hurricane)
				print('Station('..info.src..') has STORM('..comment..")")
				-- comment = comment:sub(8)
			else
		...
		while true do
			...
			if comment:sub(i+1,i+1) == 'W' then	-- Single digit !DAO!
				...
				if y and x then
					...
					comment = comment:sub(1,i-1)..comment:sub(i+5)
...
--print(string.format("%s %.6f %.6f", info.src, info.lat*60, info.lon*60))
							comment = comment:sub(1,i-1)..comment:sub(i+8)
						else
	...
	if a then
		info.alt = tonumber(a)
--print('Station('..info.src..'>'..info.dst..') has altitude='..a..' new comment '..comment)
...
function APRS:transmit(what, packet)
	print("APRS:transmit:invoking "..tostring(#transmitCallbacks).." transmitCallbacks")
...
--		if not status then print ('transmitCallback failed with '..text) end
		...
		local ok, msg = coroutine.resume( co, what, packet )
		if not ok then
			...
			print("APRS:transmit failed with\n"..tostring(msg).."\n"..full_tb)
			toast.new(msg)
		end
	end
end
...
--		if not status then print ('receiveCallback failed with '..text) end
		local co = coroutine.create( c )
		local ok, msg = coroutine.resume( co, packet, port )
		if not ok then
			local full_tb = debug.traceback( co )
						 .. "\n" .. debug.traceback( ) -- with 'stack traceback:' line
						 -- .. debug.traceback( ):sub( 17 ) -- drop 'stack traceback:' line
			print("APRS:received failed with\n"..tostring(msg).."\n"..full_tb)
			toast.new(msg)
		end
	end
end
