...
nextCmd = function()
	if type(doingCmds) ~=  'table' or #doingCmds < 1 then
		commanding = false
		doingCmds = nil
		setKissUpdate("Commands("..tostring(doingWhat)..") Complete")
		doingWhat = "nothing"
...
function BTPort:doCmds(which, callback, what)
	if not which then
		callback()
...
local function IGate(p)
	...
	if APRSIS and config.Bluetooth.IGate then
		APRSIS:sendPacket(p)
	else print("NOT IGating:APRISS="..tostring(APRSIS).." enabled="..tostring(config.Bluetooth.IGate))
	end
	...
	--														print("Bluetooth:KISS:"..KISSFormatHex(line))
										...
										elseif what == 'state' then
											...
											if text == 'Connected' then
												BTPort:doCmds(openCmds, function() end, "openCmds")
											end
										elseif what == 'disconnect' then
											print("Bluetooth:"..tostring(what)..":"..tostring(text))
											if config.Bluetooth and config.Bluetooth.Enabled and config.Bluetooth.AutoReconnect then
												print("AutoReconnect:setBluetoothEnabled("..tostring(config.Bluetooth.Enabled)..")="..tostring(MOAIAppAndroid.setBluetoothEnabled(config.Bluetooth.Enabled)))
											end
										else
											print("Bluetooth:"..tostring(what)..":"..tostring(text))
											setKissUpdate(tostring(what)..":"..tostring(text))
										...
										BTPort:doCmds(preCmds, function()
															if xmitQueue then
																...
																BTPort:doCmds(postCmds, function()
																							if xmitQueue and #xmitQueue > 0 then
																								print("Bluetooth:Dumping "..tostring(#xmitQueue).." Transmit packet(s)")
																							end
																							xmitQueue = nil
																						end, "postCmds")
															else print("Bluetooth:Transmitting "..tostring(xmitQueue).." packet(s)")
															end
														end, "preCmds")
									end
								end
