...
local function updateUI()
	...
	if IPFSUpdate and IPFStext then
		IPFSUpdate = setAndSize(IPFStext, IPFSUpdate)
	end
...
local serviceCoroutines = {}

function runServiceCoroutine(func, ...)
	local info = debug.getinfo( 2, "Sl" )
	local where = info.source..':'..info.currentline
	if where:sub(1,1) == '@' then where = where:sub(2) end

	--print("runServiceCoroutine:"..tostring(func).." coroutine="..tostring(coroutine).." create="..tostring(coroutine.create).." from:"..where)
	local args = {...}
	local v = coroutine.create(func)
	print("runServiceCoroutine:"..tostring(func).." coroutine="..tostring(v).." from:"..where)
	local status, values = coroutine.resume(v, unpack(args))
	if not status then
		local full_tb = tostring(debug.traceback( serviceCoroutines[tostring(v)] ))
					 .. "\n" .. tostring(debug.traceback( )) -- with 'stack traceback:' line
					 -- .. debug.traceback( ):sub( 17 ) -- drop 'stack traceback:' line
		print('runServiceCoroutine['..tostring(v)..'] failed with '..tostring(values).."\n"..full_tb)
	end
	serviceCoroutines[tostring(v)] = v
end

local function runServiceCoroutines()
	local which = ''
	local now = MOAISim.getDeviceTime()
	local dead = {}
	local ready = {}
--print(string.format("Running %d serviceCoroutines", #serviceCoroutines))
	for k,v in pairs(serviceCoroutines) do
		local status = coroutine.status(v)
		if status == "dead" then
			dead[#dead+1] = k
		elseif status == "suspended" then
			ready[#ready+1] = k
		else print("serviceCoroutine["..tostring(k).."].status="..tostring(status))
		end
	end
	local elapsed = (MOAISim.getDeviceTime()-now)*1000
	which = 'prep:'..math.floor(elapsed)..'ms'
...
print('runServiceCoroutines['..v..'] DIED!')
		serviceCoroutines[v] = nil
	end

	serviceActive = true
	for k,v in pairs(ready) do
--print('runServiceCoroutines['..k..'] dispatching '..tostring(v))
		local start = MOAISim.getDeviceTime()
		local status, values = coroutine.resume(serviceCoroutines[v])
		if not status then
			local full_tb = debug.traceback( serviceCoroutines[v] )
						 .. "\n" .. debug.traceback( ) -- with 'stack traceback:' line
						 -- .. debug.traceback( ):sub( 17 ) -- drop 'stack traceback:' line
			print('runServiceCoroutines['..k..'] failed with '..tostring(values).."\n"..full_tb)
		end
		local elapsed = (MOAISim.getDeviceTime()-start)*1000
--		accumulateThreadRun(k, elapsed)
		which = which..' '..k..':'..math.floor(elapsed)..'ms'
...
local gpxDir = (MOAIEnvironment.externalFilesDirectory or '/sdcard/'..tostring(MOAIEnvironment.appDisplayName))..'/GPX/'
...
local function runService()
		...
		if elapsed > 67 and true then
			print(string.format('runServiceTimers took %.2fms busy %.2f%% doing %s',
							elapsed,
							totalElapsed * 100 / ((MOAISim.getDeviceTime()-lastPrint)*1000),
							tostring(which)))
		end
		--print("runService invoking runServiceCoroutines")
		local start2 = MOAISim.getDeviceTime()
		local which2 = runServiceCoroutines()
		local elapsed2 = (MOAISim.getDeviceTime() - start2) * 1000
		if false or elapsed2 > 67 and true then
			print(string.format('runServiceCoroutines took %.2fms doing %s, simRunning:%s',
							elapsed2, tostring(which2), tostring(simRunning)))
		end
...
--		end
	...
	elseif req.puri.path == "/APRSISMO.code" then
		if not req.puri or not req.puri.pquery or not req.puri.pquery.code then
			return spoon.response(200, string.format("<H1>Nothing To See Here</H1><p>But here's what you told me!</p><table border><tr><td>Opt<td>%s<tr><td>Req<td>%s<tr><td>Method<td>%s<tr><td>URI<td>%s<tr><td>Ver<td>%s<tr><td>Headers<td>%s<tr><td>puri<td>%s<tr><td>puri.ppath<td>%s<tr><td>puri.pquery<td>%s",
									printableTable(opt,"<BR>"), printableTable(req,"<BR>"),
									tostring(req.method), tostring(req.uri), tostring(req.httpver),
									printableTable(req.headers,"<BR>"), printableTable(req.puri,"<BR>"),
			...
			config.Dropbox.AccessCode = req.puri.pquery.code
--[[
...
{"access_token": "DXkqwNYZX-gAAAAAAAAjvQJrTnJFncTJyCID6K_h4_BDkwqZitWXbgXXQ1UsuAw-", "token_type": "bearer", "uid": "74000021", "account_id": "dbid:AABYlPM5mdwQ-CVkOtI15EDtV7emHfNxgIE"}
			...
			task:setUrl ( "https://api.dropbox.com/oauth2/token" )
			--task:setUrl ( "http://localhost:6362" )
			task:setHeader ( "authorization", "Basic "..MOAIDataBuffer.base64Encode("f2xxipuqrcaytsu"..":".."n7a86vnm3o6kmbg") )
			local body = "code="..config.Dropbox.AccessCode.."&grant_type=authorization_code&redirect_uri=http://localhost:6362/APRSISMO.code"
			print("Body="..body)
			task:setBody ( body )
			task:setStream ( stream )
			task:setTimeout ( 10 )
...
--			end
			...
			if content then
				...
				elseif type(values) == 'table' then
					print(printableTable('DropboxCode:json', values))
					if values.access_token then
						config.Dropbox.AccessToken = values.access_token
						config.Dropbox.token_type = values.token_type
						config.Dropbox.uid = values.uid
						config.Dropbox.account_id = values.account_id
					end
				end

				return spoon.response(200, content, "text/plain")
			else
				return spoon.response(200, string.format("<H1>Error Getting Token From Code</H1><p>But here's what you told me!</p><table border><tr><td>Opt<td>%s<tr><td>Req<td>%s<tr><td>Method<td>%s<tr><td>URI<td>%s<tr><td>Ver<td>%s<tr><td>Headers<td>%s<tr><td>puri<td>%s<tr><td>puri.ppath<td>%s<tr><td>puri.pquery<td>%s",
										printableTable(opt,"<BR>"), printableTable(req,"<BR>"),
										tostring(req.method), tostring(req.uri), tostring(req.httpver),
										printableTable(req.headers,"<BR>"), printableTable(req.puri,"<BR>"),
										printableTable(req.puri.ppath,"<BR>"), printableTable(req.puri.pquery,"<BR>")))
...
local function POST(opt, req)

	print("POST "..tostring(req.puri.path))
...
--MOAICoroutine.new ():run ( function()
	runServiceCoroutine ( function()
								spoon.debug = false
								spoon.spoon({verbose=false, loop=true, port=6362, GET=GET, POST=POST})
							end)
