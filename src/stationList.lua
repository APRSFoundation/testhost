...
local function doShowPolygon(polygon, ID, lat, lon)
	if showing then
	
		if not polygon.minSize then print(string.format("stationlist:doShowPolygon(%s) minSize=%s", tostring(ID), tostring(polygon.minSize))) end
		osmTiles:showPolygon(polygon, ID)
--[[
		if lat and lon and polygon.filled and ID ~= "TstOvrLap" and ID ~= "NULL-IS" then
			if ID == "MHXTOREKA" or ID == "MHXTORERA" then
				osmTiles:moveTo(lat, lon)
			else
				local loadCount = osmTiles:getQueueStats()
				if loadCount == 0 and not osmTiles:crosshairActive() then
--		if lat and lon and polygon.filled and ID == "FLC009" then
--		if lat and lon and polygon.filled and ID == "TstOvrLap" then
					--osmTiles:moveTo(lat, lon)
				else
					print(string.format("Not Centering %s, %d Tiles Pending", ID, loadCount))
				end
			end
		end
		]]
...
function relabelME(text)
	if myStation.symbolLabel and myStation.symbolLabel.label.inGroup then
		local newText = myStation.stationID.."\n<time>"..tostring(text).."</>"
		if newText ~= myStation.symbolLabel.label:getString() then
			myStation.symbolLabel.label:setString(newText)
		end
	end
end
...
function M:clearOldStations(ageInSeconds, confirmRtn)
	local now = os.time()
	local oldestStation = now - ageInSeconds
	if type(confirmRtn) ~= 'function' then
		print(string.format("stationList:Invalid type(confirmRtn)=%s, should be function", type(confirmRtn)))
		confirmRtn = nil
	end
print("stationList:Purging stations before "..os.date("!%H:%M:%S", oldestStation))
	local start = MOAISim.getDeviceTime()
	local clears, clearCount, skipCount = {}, 0, 0
	local clearTime = 0
	for c,station in pairs(stationInfo) do
		if station ~= centerStation and station ~= myStation then
			if station.killed or (station.lastHeard and station.lastHeard < oldestStation) then
				if not confirmRtn or confirmRtn(station) then
					clears[c] = station
	...
	if clearCount > 0 then
		text = text.." Cleared "..tostring(clearCount).." Stations in "..math.floor(clearTime*1000).."/"..math.floor((done-start)*1000).."ms"
		if skipCount > 0 then
			text = text.." Skipped "..tostring(skipCount)
		end
	end
	print(text)
	--toast.new(text, 10000)
	return clearCount, text, skipCount
end
...
--second, minute, hour, dayStations
	...
	local function relabelStation(station, now, elapsedFunction, force)
		--if force then print(string.format("FORCE relabelStation(%s) symbolLabel:%s center:%s", station.stationID, tostring(station.symbolLabel), tostring(station==centerStation))) end
		local overflow = false
		if station.symbolLabel and station.lastHeard and (force or station.symbolLabel.label.inGroup) then
...
--print(station.stationID.." elapsed "..tostring(elapsed).." overflow "..tostring(overflow))
			...
			if station == centerStation then
				newText = (station.label and station.label or station.stationID).."\n<time>"..tostring(elapsed).."</>"
			end
			if force or newText ~= station.symbolLabel.label:getString() then
				station.symbolLabel.label:setString(newText)
				if force then
					station.symbolLabel.label:fitSize()
					station.symbolLabel.label.xOffset = station.symbolLabel.label:getWidth()/2 + station.symbolLabel.symbol.width / 2 * station.symbolLabel.symbol.scale
				end
			end
		end
		return overflow
	end
	local function updateStationTime(station)
		local now = os.time()
		if secondStations[station.stationID] then
			relabelStation(station, now, secondsElapsed, true)
		elseif minuteStations[station.stationID] then
			relabelStation(station, now, minutesElapsed, true)
		elseif hourStations[station.stationID] then
			relabelStation(station, now, hoursElapsed, true)
		elseif dayStations[station.stationID] then
			relabelStation(station, now, dayssElapsed, true)
		else
			if station.lastHeard then print("updateStationTime:Station("..tostring(station.stationID)..") not in any timing list but lastHeard?") end
		end
	end
...
if elapsed > 10 then print(string.format('UpdateSecondStations:Scanned %d Removed %d took %.2fmsec', scanned, removed, elapsed)) end
					end, 0)	-- do it forever
...
--print("Moving "..c.." from hour to day")
	...
	performWithDelay2("PurgeStationTracks", 5*60*1000, function()
							if stationCount > 10 then	-- Don't bother for low station counts
								...
								--if config.StationID == 'KJ4ERJ-DV' then oldestStation = now - 10*60 end
...
print(tostring(c)..":"..station.owner.." killed:"..tostring(station.killed))
											clears[c] = station
										elseif station.lastHeard and station.lastHeard < oldestStation then
--print(tostring(c).." lastHeard:"..tostring(station.lastHeard).."<"..oldestStation)
								...
								local done = MOAISim.getDeviceTime()
								local text = os.date("%H:%M:%S")..":Purging tracks for "..stationCount.." stations removed "..removed.."/"..total.." from "..from.." stations in "..math.floor(purgeTime*1000).."/"..math.floor((done-start)*1000).."ms"
								if clearCount > 0 then
...
--[[
	performWithDelay2("ScanStationList", 1000, function()
								local now = os.time()
								local oldestStation = now - retainStation
								local clears = {}
								for c,station in pairs(stationInfo) do
...
function M:getStation(ID)
	return stationInfo[ID]
end

function M:moveTo(lat, lon, force)
	if force or not osmTiles:crosshairActive() then
		osmTiles:moveTo(lat, lon)
	end
end
...
function M:updateCenterStation(station)
	if station and station ~= centerStation then
		local oldCenter = centerStation
		centerStation = station
		if oldCenter then
			updateStationTime(oldCenter)
			doShowSymbol(oldCenter.lat, oldCenter.lon, oldCenter.symbolLabel, oldCenter.stationID)	-- Adjust for new offset
		end
		if centerStation then updateStationTime(centerStation) end
...
--print('updateCenterStation('..centerStation.stationID..') moveTo '..centerStation.lat..' '..centerStation.lon)
	...
	if osmTiles then
		if not simRunning then
			if debugging then
				osmTiles:ensureTiles(centerStation.lat, centerStation.lon)
			else
				local status, text = pcall(osmTiles.ensureTiles, osmTiles, centerStation.lat, centerStation.lon)
				if not status then print ('osmTiles:ensureTiles failed with '..text) end
			end
		else
			--print("need to remove ensureTiles!")
			--osmTiles:ensureTiles(centerStation.lat, centerStation.lon)
			if debugging then
...
function moveME--[[Really]](newLat, newLon, newAlt, newCourse, newSpeed, newAcc)
	...
	--print("Moving me "..(myStation.gotLocation and "GOT" or "None"))

	if myStation.gotLocation then	-- Only count distance travelled after first point
		...
		odometer = odometer + deltaDistance
		--print(string.format('Adding %ift to odometer giving %ift simRunning:%s',
		--					deltaDistance*5280, odometer*5280, tostring(simRunning)))
		if odotext then
...
--print('getSymbolLabel('..symbol..') for:'..stationID..' name:'..symbols:getSymbolName(symbol))
	...
	--if shouldIShowIt(stationID) then
		...
		--local label = TextBackground { text=stationID, align = {"left", "center"}, }
--print("getSymbolLabel("..symbol..") for "..stationID.." labelling3...size:"..tostring(symbolLabel.symbol.width)..'x'..tostring(symbolLabel.symbol.height).." scale:"..tostring(symbolLabel.symbol.scale))
		...
		label:setStyle('comment', newStyle(font, math.ceil(label.orgTextSize*7/8), 0, 0, 0))
...
--print(stationID..' '..label.orgTextSize..'pts height '..label:getHeight()..' width '..label:getWidth()..' xoffset '..label.xOffset..' bounds:'..xMin..','..yMin..'->'..xMax..','..yMax..' or '..(xMax-xMin)..'x'..(yMax-yMin))
		...
		addTime("Lbl", MOAISim.getDeviceTime()-start)
...
function M.getZoom()
	if osmTiles then
		return osmTiles:getZoom()
	end
	return 0
end

function M.packetReceived(line, port, callerMoveTo)
	-- if callerMoveTo then print("syslog:callerMoveTo:"..tostring(line)) end
	local start = MOAISim.getDeviceTime()
	initTime()
...
--							station.telemetry.digital or '0'))
		...
		if packetInfo.weather then
			station.weather = packetInfo.weather
			if not station.weatherPackets then station.weatherPackets = 0 end
			station.weatherPackets = station.weatherPackets + 1
			station.weatherTime = os.time()
		end
		if packetInfo.PHG then
			if packetInfo.PHG.Power == 0 and packetInfo.PHG.Height == 0
			and packetInfo.PHG.Gain == 0 and packetInfo.PHG.Directivity == 0 then
				station.PHG = nil
print(printableTable("zeroPHG",packetInfo.PHG))
			else station.PHG = packetInfo.PHG
--print(printableTable(station.stationID.."-PHG",packetInfo.PHG))
			end
		end

		if packetInfo.dst ~= "*MicE*" then station.tocall = packetInfo.dst end
		if packetInfo.alt then station.alt = packetInfo.alt end
		if packetInfo.range then station.range = packetInfo.range end
	...
	DidWhat = badMove
				...
				if not duplicate then
					...
					if deltaSec < 5 and packetInfo.src ~= config.StationID then
						local start = MOAISim.getDeviceTime()
...
-- print("stationList:packetReceived:"..stationID.." moved "..deltaDistance.."mi in "..deltaSec.."sec or "..speed.."mph")
							if speed > 500
							and packetInfo.src ~= config.StationID
...
--[[
			{	double distance, bearing;
				...
				if not badMove and moved then
					...
					if packetInfo.speed then station.pktSpeed = packetInfo.speed end
					if packetInfo.course then station.pktCourse = packetInfo.course end
...
if showing then
					...
					updateStationTime(station)	-- This forces a re-size on the label as well
			...
			if type(packetInfo.comment) == 'string' then
				local start = MOAISim.getDeviceTime()
				local s, e, lt, ot, sc, t, id  = string.find(packetInfo.comment, " }(.)(.)(.)(%S-){(.?.?.?.?.?)")	-- Check it for MultiLine
--if station.stationID == "ISS" then
--	print(string.format("%s s:%s e:%s lt:%s ot:%s sc:%s t:%s id:%s from %s",
--						station.stationID, tostring(s), tostring(e), tostring(lt), tostring(ot), tostring(sc), tostring(t), tostring(id),
--						tostring(packetInfo.comment)))
--end
				if s then
					packetInfo.comment = packetInfo.comment:sub(1,s)..packetInfo.comment:sub(e+1)
...
--					print(printableTable(station.stationID.."MLP", points, " "))
					local color
					local on = 255	-- 192
					if lt == 'a' or lt == 'b' or lt == 'c' then color = {on,0,0}
					elseif  lt == 'd' or lt == 'e' or lt == 'f' then color = {on,on,0}
					elseif  lt == 'g' or lt == 'h' or lt == 'i' then color = {0,0,on}
					elseif  lt == 'j' or lt == 'k' or lt == 'l' then color = {0,on,0}
...
--[[
					...
					if filled then
						station.polygon.alpha = config.Screen.NWSOpacity / 100
					else station.polygon.alpha = 1.0
					end
					station.polygon.filled = filled
					station.polygon.minSize = symbolSize
					doShowPolygon(station.polygon, station.stationID, station.lat, station.lon)
					
					if id and id ~= "" then	-- this makes it possibly an NWS packet
						print("Possible NWS MultiLine: ID="..tostring(id))
					end
				end
			...
			if type(packetInfo.comment) == 'string' then
				local start = MOAISim.getDeviceTime()
				if packetInfo.comment ~= "" then station.comment = packetInfo.comment end
				do	-- Hide the local variables and functions

					local function escape(s)
						return s:gsub("<", "<<")
					end
					local function getCompassPoint(degrees, value, text)
						if value and degrees == value then return text end
						local headings = { "N", "NNE", "NE", "ENE",
											"E", "ESE", "SE", "SSE",
											"S", "SSW", "SW", "WSW",
											"W", "WNW", "NW", "NNW" }
						local index = degrees + (360/#headings/2)	-- Round up by 1/2 interval
						index = math.floor(index * #headings / 360)
						if index >= 0 and index < #headings then
							return headings[index+1]
						else return string.format("%d°", degrees)
						end
					end
					
					local function addLine(to, new)
						if new == "" then return to end
						if to ~= "" then to = to.."\n" end
						to = to..new
						return to
					end
					local function firstLine()
						local result = escape(station.stationID)
						if station.owner then
							result = result.." de "..escape(station.owner)
						end
						if station.tocall then
							result = result..">"..escape(station.tocall)
						end
						if station.killed then
							result = "*** KILLED ***\n"..result
						end
						return result
					end
					local function comment(what)
						if not what or what == "" then return "" end
						return "<comment>"..escape(what).."</>"
					end
					local function phgRange()
						local result = ""
						if station.PHG then
							result = string.format("%dw %dft %ddBi %s",
													station.PHG.Power, station.PHG.Height,
													station.PHG.Gain, getCompassPoint(station.PHG.Directivity, 0, "Omni"))
							if station.range then
								result = result..string.format(" (%dmi)", station.range)
							end
						elseif station.range then
							result = string.format("Range: %dmi", station.range)
						end
						return result
					end
					local function altitude()
						local result = ""
						if station.alt then
							result = string.format("Altitude: %dft", station.alt)
						end
						return result
					end
					local function speed()
						local speedCount = 0
						local pktSpeed, avgSpeed, calcSpeed
						if station.pktCourse and station.pktSpeed then
							speedCount = speedCount + 1
							pktSpeed = string.format("%d %s", station.pktSpeed, getCompassPoint(station.pktCourse))
						end
						if station.speedCalculated and station.courseCalculated then
							speedCount = speedCount + 1
							calcSpeed = string.format("%d %s", station.speedCalculated, getCompassPoint(station.courseCalculated))
						end
						local result
						if speedCount > 1 then
							result = string.format("Speed: Pkt: %s Calc: %s", pktSpeed, calcSpeed)
						else
							if pktSpeed then
								result = "Pkt Speed: "..pktSpeed
							elseif calcSpeed then
								result = "Calc Speed: "..calcSpeed
							else
								result = ""
							end
						end
						if station.speedAverage then
							result = result..string.format(" Avg: %d", station.speedAverage)
						end
						return result
					end
					local function telemetry()
						if not station.telemetry then return "" end
						local result = string.format("Telem: %s (%d)",
													os.date("%x %X", station.telemetryTime),
													station.telemetryPackets)
						local values = ""
						for k,v in ipairs(station.telemetry.values) do
							if values ~= "" then values = values.." " end
							values = values..tostring(v)
						end
						result = addLine(result,"seq("..tostring(station.telemetry.seq).."): "..values)
						if station.telemetry.digital then
							result = addLine(result,string.format("bits: %08X", station.telemetry.digital))
						end
						-- print(printableTable(station.stationID.."-telemtry",station.telemetry))
						return result
					end
					local function weather()
						if not station.weather then return "" end
						local result = string.format("Weather: %s (%d)", os.date("%x %X", station.weatherTime), station.weatherPackets)
						if station.weather.windspeed and station.weather.direction then
							result = addLine(result, string.format("Wind: %dmph %s", station.weather.windspeed, getCompassPoint(station.weather.direction)))
							if station.weather.gust then
								result = result..string.format(" (Gust:%d)", station.weather.gust)
							end
						end
						local ignores = { windspeed=true, direction=true, gust=true }
						for k,v in pairs(station.weather) do
							if not ignores[k] then
								result = addLine(result, tostring(k)..": "..tostring(v))
							end
						end
						return result
					end
					local label = firstLine()
					label = addLine(label, string.format("Sym: %s (%s)", symbols:getSymbolName(station.symbol), station.symbol))
					label = addLine(label, comment(station.comment))
					label = addLine(label, comment(station.statusReport))
					label = addLine(label, phgRange())
					label = addLine(label, telemetry())
					label = addLine(label, weather())
					label = addLine(label, speed())
					label = addLine(label, altitude())
					local newLabel = station.label ~= label
					station.label = label
					if newLabel and station == centerStation then
						updateStationTime(station)	-- This forces a re-size on the label as well
					end
				end
			...
			then
				...
				station.polygon.alpha = config.Screen.DFOpacity / 100
				station.polygon.filled = true
				station.polygon.minSize = symbolSize
				print("DF Station:"..station.stationID)
				doShowPolygon(station.polygon, station.stationID, station.lat, station.lon)
			...
			if station == centerStation then
				if not osmTiles:crosshairActive() or not simRunning then
					local start = MOAISim.getDeviceTime()
...
if Application:isDesktop() and callerMoveTo and not osmTiles:crosshairActive() then
	--M:updateCenterStation(station)	-- Doesn't update label size?
	local clat, clon, zoom = osmTiles:getCenter()
	local fromPoint = LatLon.new(clat, clon)
	local toPoint = LatLon.new(station.lat, station.lon)
	local distance = kmToMiles(fromPoint.distanceTo(toPoint))
	local hdist, vdist = osmTiles:rangeLatLon()
	local scale = math.min(hdist, vdist)
	if tonumber(callerMoveTo) then
		osmTiles:moveTo(station.lat, station.lon, tonumber(callerMoveTo))
	elseif distance > scale * 3 / 4 and osmTiles:getZoom() > 3 then	-- Don't move the screen around unnecessarily
		osmTiles:moveTo(station.lat, station.lon)
	end
--[[
elseif Application:isDesktop() and not osmTiles:crosshairActive() and centerStation == myStation then
	local count = osmTiles:getQueueStats()
	if count == 0 or station.symbol == "\\S" then
		osmTiles:moveTo(station.lat, station.lon)
	end
]]
end
...
--/* b) Sequencenumber (after '{') has 5 alphanumeric characters */
			elseif false and packetInfo.src:match("^%u%u%u%u%u%u$") and msg.ack:match("^%w%w%w%w%w$") then
--MEGWSW>APRS,qAo,WE7U-3::NWS_CANCL:260600z,WINTER_WEATHER,ARZ026>028-035-036-048-049-058-MOZ113-115 {Q3KAA
				print("NWS Message from "..tostring(packetInfo.src).." to "..msg.addressee.." body:"..msg.text)
				local when, what, where = msg.text:match("^(%d%d%d%d%d%dz),(.-),(.+)$")
				print("NWS Time:"..tostring(when).." Type:"..tostring(what).." IDs:"..tostring(where))
				local function isdigit(c) return c>='0' and c<='9' end
				local function strtol(s,i)
					local e = i
					while e <= #s and isdigit(s:sub(e,e)) do
						e = e + 1
					end
				...
				local state
				while z <= #where do
					if not isdigit(where:sub(z,z)) then
						local last = z
						while last <= #where and not isdigit(where:sub(last,last)) do
							last = last + 1
						end
						state = where:sub(z,last-1)
						z = last
					end
					if z <= #where then
						local vs, ve, n
						vs, n = strtol(where,z)
						if where:sub(n,n) == '>' then	-- It's a range nnn>nnn
							ve, n = strtol(where,n+1)
						else ve = vs
						end
						if ve >= vs then
							for v=vs,ve do
								local ID = string.format("%s%03d", state, v)
								
								if not ActiveShapes then ActiveShapes = {} end
								if not ActiveShapes[ID] then
--[[				if lt == 'a' or lt == 'b' or lt == 'c' then color = {192,0,0}
					...
					elseif  lt == 'j' or lt == 'k' or lt == 'l' then color = {0,192,0}
									...
									require ("testshape")
									local poly = addPolygons(ID, {192,0,0})	-- {250,250,224} = light yellow
									print("NWS:ShapeID("..ID..") Fixing closure on "..tostring(#poly).." polygons for "..ID)
									for p=1,#poly do
										local points = poly[p]	-- ensure last point == first point for closure
										if (points[1] ~= points[#points-1] or points[2] ~= points[#points]) then
											points[#points+1] = points[1]
											points[#points+1] = points[2]
										end
										--print("NWS:"..ID.." Shape["..tostring(p).."] has "..tostring(#points/2).." points")
									end
									poly.filled = true
									poly.color = {192,0,0}
									poly.lineWidth = config.Screen.scale * 4	-- Basically 2 pixesl?
									poly.alpha = config.Screen.NWSOpacity / 100
									poly.minSize = symbolSize
									doShowPolygon(poly, ID, poly.lat, poly.lon)
									ActiveShapes[ID] = poly
								end

							end
						else
							print("NWS:Bad Range from "..tostring(vs).." -> "..tostring(ve))
						end
						z = n+1	-- character after separator
					end
				end
			end
...
--[[
					...
					if station.owner and station.killed then
						...
						--killCount = killCount and killCount+1 or 1
						--serviceWithDelay("kill"..killCount, 1000, function() print("Killing "..station.owner..":"..stationID) M:clearStation(station) end)
						--print("Killing "..station.owner..":"..stationID)
						M:clearStation(station)
					end
