...
function onKeyDown(event)
	...
	elseif key >= 32 and key <= 126 then
		print('textentry_scene:key='..tostring(key))
		if shifted and shiftKeys[key] then key = shiftKeys[key]
		elseif capsLock and key >= string.byte("a") and key <= string.byte("z") and shiftKeys[key] then key = shiftKeys[key]
		end
		value = value..string.char(key)
